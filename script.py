...
class Database:
    ...
    def calculate_how_many_days_to_birthday(dob):
        ...
        date_of_birthday_this_year = date_of_birthday.replace(
            year=date_of_birthday.year + age
        )
    ...
    def create_database(self):
        ...
        for number, person in enumerate(people_json):
            person_record = Person(
                ...
                password_md5=person["login"]["md5"],
                password_sha1=person["login"]["sha1"],
                password_sha256=person["login"]["sha256"],
                dob=person["dob"]["date"],
                age=person["dob"]["age"],
                registered_date=person["registered"]["date"],
                days_to_birthday=self.calculate_how_many_days_to_birthday(
                    person["dob"]["date"]
                ),
    ...
    def calculate_male_female_percentage():
        ...
        percentage_f = round(
            (female_counter / (male_counter + female_counter)) * 100, 3
        )
        ...
        if quantity.isnumeric():
            ...
            for person in all_people:
                ...
                if searching_element in unique_elements:
                    unique_elements[searching_element] = (
                        unique_elements[searching_element] + 1
                    )
                else:
                    unique_elements[searching_element] = 1
            sorted_unique_elements = sorted(
                unique_elements.items(), key=lambda x: x[1], reverse=True
            )
    ...
    def check_people_passwords(self):
        ...
        sorted_passwords = sorted(
            pointed_passwords.items(), key=lambda x: x[1], reverse=True
        )
