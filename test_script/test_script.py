...
class TestDataBase(unittest.TestCase):
    ...
    def test_database(self):
        random_person = requests.get(
            "https://randomuser.me/api/?results=1000&seed=ec14fa7f0b8242e0"
        ).json()

        self.assertIn("gender", str(random_person))
        self.assertIn("ec14fa7f0b8242e0", str(random_person))
        self.assertIn(
            "Database created from ec14fa7f0b8242e0 seed, has 1000 records", str(DB)
        )
...
class TestFunctions(unittest.TestCase):
    ...
    def test_find_most_common_elements(self):
        ...
        self.assertIn(
            "5555 is too big! In database there are only 1000 people.", too_long_result
        )
    ...
    def test_find_birthdays_between_dates(self):
        ...
        self.assertIn(
            "Date format needs to be YYYY/mm/dd (for example 2137/05/04)!",
            no_numbers_dates,
        )
